...
mutual
  inductive Formula (σ : Signature) where
    ...
    | forallN : Rules.Label → Formula σ → Formula σ
    | existsN : Rules.Label → Formula σ → Formula σ
    | forallS : Rules.Label → Formula σ → Formula σ
    | existsS : Rules.Label → Formula σ → Formula σ
  deriving Inhabited
end
...
private def eraseDupLabels : List Rules.Label → List Rules.Label := List.eraseDups

private partial def freshVar (base : Rules.Label) (avoid : List Rules.Label) : Rules.Label :=
  let rec loop (candidate : Rules.Label) : Rules.Label :=
    if candidate ∈ avoid then loop (FreshLabel.next candidate) else candidate
  loop base

mutual
  partial def NTerm.freeNVars : NTerm σ → List Rules.Label
    | .var x => [x]
    | .fn _ ns ss => eraseDupLabels (ns.flatMap NTerm.freeNVars ++ ss.flatMap STerm.freeNVars)

  partial def NTerm.freeSVars : NTerm σ → List Rules.Label
    | .var _ => []
    | .fn _ ns ss => eraseDupLabels (ns.flatMap NTerm.freeSVars ++ ss.flatMap STerm.freeSVars)

  partial def STerm.freeNVars : STerm σ → List Rules.Label
    | .var _ => []
    | .fn _ ns ss => eraseDupLabels (ns.flatMap NTerm.freeNVars ++ ss.flatMap STerm.freeNVars)

  partial def STerm.freeSVars : STerm σ → List Rules.Label
    | .var x => [x]
    | .fn _ ns ss => eraseDupLabels (ns.flatMap NTerm.freeSVars ++ ss.flatMap STerm.freeSVars)

  partial def NTerm.allNames : NTerm σ → List Rules.Label
    | .var x => [x]
    | .fn _ ns ss => eraseDupLabels (ns.flatMap NTerm.allNames ++ ss.flatMap STerm.allNames)

  partial def STerm.allNames : STerm σ → List Rules.Label
    | .var x => [x]
    | .fn _ ns ss => eraseDupLabels (ns.flatMap NTerm.allNames ++ ss.flatMap STerm.allNames)
end

mutual
  partial def Formula.freeNVars : Formula σ → List Rules.Label
    | .top => []
    | .bot => []
    | .pred _ ns ss => eraseDupLabels (ns.flatMap NTerm.freeNVars ++ ss.flatMap STerm.freeNVars)
    | .eqN t u => eraseDupLabels (t.freeNVars ++ u.freeNVars)
    | .eqS t u => eraseDupLabels (t.freeNVars ++ u.freeNVars)
    | .neg A => A.freeNVars
    | .imp A B => eraseDupLabels (A.freeNVars ++ B.freeNVars)
    | .and A B => eraseDupLabels (A.freeNVars ++ B.freeNVars)
    | .or A B => eraseDupLabels (A.freeNVars ++ B.freeNVars)
    | .forallN x A => (A.freeNVars).erase x
    | .existsN x A => (A.freeNVars).erase x
    | .forallS _ A => A.freeNVars
    | .existsS _ A => A.freeNVars

  partial def Formula.freeSVars : Formula σ → List Rules.Label
    | .top => []
    | .bot => []
    | .pred _ ns ss => eraseDupLabels (ns.flatMap NTerm.freeSVars ++ ss.flatMap STerm.freeSVars)
    | .eqN t u => eraseDupLabels (t.freeSVars ++ u.freeSVars)
    | .eqS t u => eraseDupLabels (t.freeSVars ++ u.freeSVars)
    | .neg A => A.freeSVars
    | .imp A B => eraseDupLabels (A.freeSVars ++ B.freeSVars)
    | .and A B => eraseDupLabels (A.freeSVars ++ B.freeSVars)
    | .or A B => eraseDupLabels (A.freeSVars ++ B.freeSVars)
    | .forallN _ A => A.freeSVars
    | .existsN _ A => A.freeSVars
    | .forallS x A => (A.freeSVars).erase x
    | .existsS x A => (A.freeSVars).erase x

  partial def Formula.allNames : Formula σ → List Rules.Label
    | .top => []
    | .bot => []
    | .pred _ ns ss => eraseDupLabels (ns.flatMap NTerm.allNames ++ ss.flatMap STerm.allNames)
    | .eqN t u => eraseDupLabels (t.allNames ++ u.allNames)
    | .eqS t u => eraseDupLabels (t.allNames ++ u.allNames)
    | .neg A => A.allNames
    | .imp A B => eraseDupLabels (A.allNames ++ B.allNames)
    | .and A B => eraseDupLabels (A.allNames ++ B.allNames)
    | .or A B => eraseDupLabels (A.allNames ++ B.allNames)
...
mutual
  partial def NTerm.renameNVar (old new : Rules.Label) : NTerm σ → NTerm σ
    | .var y => if y = old then .var new else .var y
    | .fn f ns ss => .fn f (ns.map (NTerm.renameNVar old new)) (ss.map (STerm.renameNVar old new))

  partial def NTerm.renameSVar (old new : Rules.Label) : NTerm σ → NTerm σ
    | .var y => .var y
    | .fn f ns ss => .fn f (ns.map (NTerm.renameSVar old new)) (ss.map (STerm.renameSVar old new))

  partial def STerm.renameNVar (old new : Rules.Label) : STerm σ → STerm σ
    | .var y => .var y
    | .fn f ns ss => .fn f (ns.map (NTerm.renameNVar old new)) (ss.map (STerm.renameNVar old new))

  partial def STerm.renameSVar (old new : Rules.Label) : STerm σ → STerm σ
    | .var y => if y = old then .var new else .var y
    | .fn f ns ss => .fn f (ns.map (NTerm.renameSVar old new)) (ss.map (STerm.renameSVar old new))
end

mutual
  partial def Formula.renameNVar (old new : Rules.Label) : Formula σ → Formula σ
    | .top => .top
    | .bot => .bot
  ...
  partial def Formula.renameSVar (old new : Rules.Label) : Formula σ → Formula σ
    | .top => .top
    | .bot => .bot
...
mutual
  partial def NTerm.substN (x : Rules.Label) (t : NTerm σ) : NTerm σ → NTerm σ
    | .var y => if x = y then t else .var y
    | .fn f ns ss => .fn f (ns.map (NTerm.substN x t)) (ss.map (STerm.substN x t))

  partial def STerm.substN (x : Rules.Label) (t : NTerm σ) : STerm σ → STerm σ
    | .var y => .var y
    | .fn f ns ss => .fn f (ns.map (NTerm.substN x t)) (ss.map (STerm.substN x t))

  partial def NTerm.substS (x : Rules.Label) (t : STerm σ) : NTerm σ → NTerm σ
    | .var y => .var y
    | .fn f ns ss => .fn f (ns.map (NTerm.substS x t)) (ss.map (STerm.substS x t))

  partial def STerm.substS (x : Rules.Label) (t : STerm σ) : STerm σ → STerm σ
    | .var y => if x = y then t else .var y
    | .fn f ns ss => .fn f (ns.map (NTerm.substS x t)) (ss.map (STerm.substS x t))
end

mutual
  partial def Formula.substN (x : Rules.Label) (t : NTerm σ) : Formula σ → Formula σ
    | .top => .top
    | .bot => .bot
  ...
  partial def Formula.substS (x : Rules.Label) (t : STerm σ) : Formula σ → Formula σ
    | .top => .top
    | .bot => .bot
...
def Antecedent.freeNVars {σ : Signature} (Γ : Antecedent σ) : List Rules.Label :=
  (Γ.map Formula.freeNVars).flatten.eraseDups

def Antecedent.freeSVars {σ : Signature} (Γ : Antecedent σ) : List Rules.Label :=
  (Γ.map Formula.freeSVars).flatten.eraseDups

def Succedent.freeNVars {σ : Signature} (Δ : Succedent σ) : List Rules.Label :=
  (Δ.map Formula.freeNVars).flatten.eraseDups

def Succedent.freeSVars {σ : Signature} (Δ : Succedent σ) : List Rules.Label :=
...
def mkStrFn? {σ : Signature} (f : σ.StrFn) (ns : List (NTerm σ)) (ss : List (STerm σ)) :
  ...
  if _h : And (ns.length = ar.1) (ss.length = ar.2) then
    ...
    throw s!"String-sort function arity mismatch: expected ({ar.1}, {ar.2})."
...
inductive Proof (σ : Signature) : Antecedent σ → Succedent σ → Type where
  ...
  | negLeft {Γ : Antecedent σ} {Δ : Succedent σ} {A : Formula σ} :
      Proof σ Γ (A :: Δ) →
      Proof σ ((¬₂ A) :: Γ) Δ
  | forallNLeft {Γ : Antecedent σ} {Δ : Succedent σ} {x : Rules.Label} {A : Formula σ} {t : NTerm σ} :
      Proof σ ((Formula.substN x t A) :: Γ) Δ →
      Proof σ ((.forallN x A) :: Γ) Δ
  | existsNLeft {Γ : Antecedent σ} {Δ : Succedent σ} {x y : Rules.Label} {A : Formula σ}
      (hy : y ∉ (Γ.freeNVars ++ Δ.freeNVars ++ Formula.freeNVars (.existsN x A))) :
      Proof σ ((Formula.substN x (.var y) A) :: Γ) Δ →
      Proof σ ((.existsN x A) :: Γ) Δ
  | forallSLeft {Γ : Antecedent σ} {Δ : Succedent σ} {x : Rules.Label} {A : Formula σ} {t : STerm σ} :
      Proof σ ((Formula.substS x t A) :: Γ) Δ →
      Proof σ ((.forallS x A) :: Γ) Δ
  | existsSLeft {Γ : Antecedent σ} {Δ : Succedent σ} {x y : Rules.Label} {A : Formula σ}
  ...
  | negRight {Γ : Antecedent σ} {Δ : Succedent σ} {A : Formula σ} :
      Proof σ (A :: Γ) Δ →
      Proof σ Γ ((¬₂ A) :: Δ)
  | forallNRight {Γ : Antecedent σ} {Δ : Succedent σ} {x y : Rules.Label} {A : Formula σ}
      (hy : y ∉ (Γ.freeNVars ++ Δ.freeNVars ++ Formula.freeNVars (.forallN x A))) :
      Proof σ Γ ((Formula.substN x (.var y) A) :: Δ) →
      Proof σ Γ ((.forallN x A) :: Δ)
  | existsNRight {Γ : Antecedent σ} {Δ : Succedent σ} {x : Rules.Label} {A : Formula σ} {t : NTerm σ} :
      Proof σ Γ ((Formula.substN x t A) :: Δ) →
      Proof σ Γ ((.existsN x A) :: Δ)
  | forallSRight {Γ : Antecedent σ} {Δ : Succedent σ} {x : Rules.Label} {A : Formula σ} {y : Rules.Label}
      (hy : y ∉ (Γ.freeSVars ++ Δ.freeSVars ++ Formula.freeSVars (.forallS x A))) :
      Proof σ Γ ((Formula.substS x (.var y) A) :: Δ) →
      Proof σ Γ ((.forallS x A) :: Δ)
  | existsSRight {Γ : Antecedent σ} {Δ : Succedent σ} {x : Rules.Label} {A : Formula σ} {t : STerm σ} :
...
example : freshVar "y" (["y", "x"] : List Rules.Label) = "y'" := by
  native_decide
...
structure Names (σ : Signature) where
  numFn : σ.NumFn → Rules.Label
  strFn : σ.StrFn → Rules.Label
  pred : σ.Pred → Rules.Label
...
def allMany : List Rules.Label → Rules.Formula → Rules.Formula
  | [], A => A
  | x :: xs, A => .all x (allMany xs A)

def exMany : List Rules.Label → Rules.Formula → Rules.Formula
  | [], A => A
...
private def vars : Rules.Label → Nat → List Rules.Label
  | _, 0 => []
  | base, n + 1 => base :: vars (FreshLabel.next base) n

private def argsHaveSorts (numVars strVars : List Rules.Label) : List Rules.Formula :=
...
def phiForPred {σ : Signature} (names : Names σ) (p : σ.Pred) : Rules.Formula :=
  ...
  let numVars := vars "n" ar.1
  let strVars := vars "s" ar.2
  let premises := [Rules.Formula.pred (α := Rules.Label) (names.pred p)
    (numVars.map (fun x => Rules.Term.var (α := Rules.Label) x) ++
      strVars.map (fun x => Rules.Term.var (α := Rules.Label) x))]
  let concl := andList (argsHaveSorts numVars strVars)
  allMany (numVars ++ strVars) (Rules.Formula.imp (andList premises) concl)

end SingleSorted

end FastProofTheory.LK2
