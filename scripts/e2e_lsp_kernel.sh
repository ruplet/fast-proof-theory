...
WIRE="/tmp/mypa_lsp_e2e_wire.txt"
OUT="/tmp/mypa_lsp_e2e_out.txt"
...
const codec = require(path.join(ROOT, 'lsp_server/out/protoCodec.js'));
...
(async () => {
  ...
  const scoped = lines.slice(0, 3).join('\n');

  const ir = parser.parseDocumentToIR(`file://${filePath}`, 1, scoped);
  const encoded = await codec.encodeDocumentDelimited(ir);
  await codec.decodeDocumentDelimited(encoded);
  fs.writeFileSync('/tmp/mypa_lsp_e2e_wire.txt', wire.documentToKernelWire(ir), 'utf8');
})();
NODE
...
const ROOT = process.cwd();
const wire = require(path.join(ROOT, 'lsp_server/out/irWire.js'));
const out = fs.readFileSync('/tmp/mypa_lsp_e2e_out.txt','utf8');
...
console.log('PASS: LSP logic + kernel smoke check');
NODE
