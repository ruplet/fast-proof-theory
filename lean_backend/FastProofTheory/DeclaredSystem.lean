...
inductive DeclaredSystem where
  ...
  | njp (fragment : FastProofTheory.NaturalDeduction.PropositionalFragment)
  | nkp (fragment : FastProofTheory.NaturalDeduction.PropositionalFragment)
  | ljp
  | lkp
  | nj
  | nk
...
def DeclaredSystem.allowsFormula : DeclaredSystem → FastProofTheory.Gentzen.Formula → Bool
  ...
  | .njp fragment, formula => FastProofTheory.NaturalDeduction.System.njp fragment |>.allowsFormula formula
  | .nkp fragment, formula => FastProofTheory.NaturalDeduction.System.nkp fragment |>.allowsFormula formula
  | .ljp, _ => false
  | .lkp, _ => false
  | .nj, _ => false
  | .nk, _ => false
  | .lj, _ => false
  | .lk, _ => false
  | .systemF, formula => FastProofTheory.NaturalDeduction.System.systemF.allowsFormula formula
...
def DeclaredSystem.hasValidConfiguration : DeclaredSystem → Bool
  ...
  | .njp fragment => (FastProofTheory.NaturalDeduction.System.njp fragment).hasValidConfiguration
  | .nkp fragment => (FastProofTheory.NaturalDeduction.System.nkp fragment).hasValidConfiguration
  | .systemF => true
  | _ => false
...
def DeclaredSystem.toNDSystem? : DeclaredSystem → Option FastProofTheory.NaturalDeduction.System
