...
import FastProofTheory.NaturalDeduction.System
...
inductive Certificate where
  ...
  | forallIntro (name : String) (body : Certificate)
  | forallElim (binder : String) (arg : Formula) (body : Formula) (proof : Certificate)
  | classical (name : String) (target : Formula) (body : Certificate)

abbrev CheckedCertificate :=
  FastProofTheory.Core.CheckedCertificate FastProofTheory.NaturalDeduction.System Goal Certificate
...
private def ensureSystem (system : FastProofTheory.NaturalDeduction.System) : Except KernelError Unit := do
  unless system.hasValidConfiguration do
    throw (.ruleNotAllowed system.displayName "ND")

private partial def ensureFormulaAllowed (system : FastProofTheory.NaturalDeduction.System) (formula : Formula) :
    Except KernelError Unit := do
  unless system.allowsFormula formula do
    throw (.unsupportedFormula system.displayName formula)

private def ensureGoalSupported (system : FastProofTheory.NaturalDeduction.System) (goal : Goal) :
    Except KernelError Unit := do
  ensureFormulaAllowed system goal.target
  goal.context.forM (fun binding => ensureFormulaAllowed system binding.formula)

private def formulaFreeVars : Formula → List String
  | .atom name => [name]
...
private def formulaAllNames : Formula → List String
  | .atom name => [name]
...
private def contextFreeVars (ctx : Context) : List String :=
  ctx.map (·.formula) |>.flatMap formulaFreeVars |>.eraseDups

private partial def freshName (base : String) (avoid : List String) : String :=
  let rec loop (candidate : String) : String :=
    if candidate ∈ avoid then loop (candidate ++ "'") else candidate
  loop base

private partial def renameTermVar (old new : String) : Rules.Term → Rules.Term
...
private partial def substFormulaAvoidCapture (name : String) (replacement : Formula) : Formula → Formula
...
private def ensureFreshForAllIntro (system : FastProofTheory.NaturalDeduction.System) (goal : Goal) (binder : String) :
    Except KernelError Unit := do
...
private partial def checkCore (system : FastProofTheory.NaturalDeduction.System) (goal : Goal) (certificate : Certificate) :
    Except KernelError Unit := do
...
private def checkCertificateRaw (system : FastProofTheory.NaturalDeduction.System) (goal : Goal) (certificate : Certificate) :
...
instance : FastProofTheory.Core.ProofKernel FastProofTheory.NaturalDeduction.System Goal Certificate KernelError where
  kind := .naturalDeduction
  check := checkCertificateRaw
  renderError := renderKernelError

def checkCertificate (system : FastProofTheory.NaturalDeduction.System) (goal : Goal) (certificate : Certificate) :
    Except KernelError CheckedCertificate :=
  FastProofTheory.Core.checkCertificate system goal certificate

def checkClosedTheorem (system : FastProofTheory.NaturalDeduction.System) (target : Formula) (certificate : Certificate) :
    Except KernelError CheckedCertificate :=
